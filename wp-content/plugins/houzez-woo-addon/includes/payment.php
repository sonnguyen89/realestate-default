	...
	class Houzez_Woo_Payment {
	    ...
	    function houzez_per_listing_payment($listing_id, $is_featured ) {
	        ...
	        if( $is_featured == 1 ) {
	        	...
	        	$product_title = sprintf( esc_html__('Upgrade to "Featured" for Listing "%s" with id %s', 'houzez-woo'), get_the_title($listing_id),$listing_id);

	        } else {

	        	$listing_price = houzez_option('price_listing_submission');
	        	$product_title = sprintf( esc_html__('Payment for Listing "%s" with id %s', 'houzez-woo'), get_the_title($listing_id),$listing_id);
	        }
